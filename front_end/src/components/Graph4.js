...
function Graph4({ districtId }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (districtId) {
        ...
        const response = await fetch(`/api/momentum?district_id=${districtId}`);
        const data = await response.json();
        const labels = data.map(item => item.year_month);
        const values = data.map(item => item.momentum);
  ...
  return (
    <div className="rectangle">
      <h2 className="text-style">모멘텀</h2>
